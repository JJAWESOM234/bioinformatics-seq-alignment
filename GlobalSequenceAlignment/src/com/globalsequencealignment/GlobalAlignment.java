...
public class GlobalAlignment
{
	...
	private final double GAP_OPEN_PENALTY = -1.0;
	...
	// Matrices to track scoring of matches or mismatchs and the overall score of each alignment
	...
	protected int[][] fillSequenceMatrix()
	{
		...
		// Loop through the columns and set the cell value to gap penalty * coulmns index
	...
	protected String[] getSequences()
	{
		...
		// loop through the sequence matrix to find the optimal path (represents the alignment) 
		while (i != 0 && j != 0)
		{
			...
			// Left case, where the 2nd sequence residue remains and a gap is inserted into the 1st sequence position
			...
			// Top case, where the 1st sequence residue remains and a gap is inserted into the 2nd sequence position
			else
			{
				returnSeq1 = this.sequence1Char[i - 1] + returnSeq1;
		...
		// If one of the sequences is out of resiude before the other this fills in the rest of the sequence with gaps 
		if (i == 0)
		{
			for (int k = 0; k < j; k++)
			{
